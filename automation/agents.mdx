...
The agent has a small dashboard you open in a browser **on the machine where the agent is installed**. Use it to enter the registration token, check the agent's connection status, and see the devices it's handling.

By default the dashboard listens on port **8080**. If that port is already used by something else on the machine, open **Settings → Dashboard Port**, enter a different port, and save — the agent restarts and the page follows it to the new address.

<Note>
  If the agent can't start because its port is already taken, start it once with `--port <number>` to reach the dashboard again, then change the port from **Settings**.
</Note>
